...
function math_replacements(s)
    ...
    s = replace(s, "exp(t)*cos(pi*(-4*t + 2*x))" => "a9")
    s = replace(s, "(eta0 + 2.0*a2 + 5.0)" => "a10")
    s = replace(s, "sqrt(g*a10)" => "a11")
    s = replace(s, "(0.2*eta0 + 0.4*a2 + 1)" => "a12")
    s = replace(s, "alpha*a11*a12^2" => "a13")
    s = replace(s, "sqrt(a13)" => "a14")
    s = replace(s, "(-1.0*pi*a13*a1/a10 - 0.8*pi*alpha*a11*a12*a1)" => "a15")
    s = replace(s, "(-20.0*pi^2*a14*a9 - 10.0*pi*a14*a15*exp(t)*a7/(a13))" => "a16")
    s = replace(s, "(-2*pi*exp(t)*a7 - 4.0*pi*a1)" => "a17")
    s = replace(s, "(a9 + 2.0*a2 + 5.0)" => "a18")
    s = replace(s, "a17*a8*a3 + 2*pi*a18*a8*a4" => "a19")
    s = replace(s, "a14*(40.0*pi^3*a14*exp(t)*a7 - 40.0*pi^2*a14*a15*a9/(a13) - 20.0*pi^2*a14*a15*exp(t)*a1*a7/(a13*a10) - 16.0*pi^2*a14*a15*exp(t)*a1*a7/(alpha*a11*a12^3) - 10.0*pi*a14*(-2.0*pi^2*a13*a2/a10 - 1.6*pi^2*alpha*a11*a12*a2 + 3.2*pi^2*alpha*a11*a12*a1^2/a10 + 0.56*pi^2*alpha*a11*a1^2)*exp(t)*a7/(a13) - 10.0*pi*a14*a15^2*exp(t)*a7/(alpha^2*g*a12^4*a10))" => "a20")
    return s
end
